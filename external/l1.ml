...
(* Este arquivo contém um interpretador, um verificador de tipo e um 
   pretty-printer para a linguagem L1. Para usá-lo copiei o arquivo 
   para um diretório de trabalho e prompt digite

       mosml -P full l1.ml 

   Isto inicia uma sessão em Moscow ML. Digitando
   ...
   Para testar outras expressões de L1 a árvore de sintaxe abstrata deve 
   ser construida explicitamente, por exemplo para avaliar a expressão
   l1 := 3 ; !l1 na memória com endereço l1 contendo o inteiro 0:
...
(* **********************************)
(* interpretador para a semântica   *)
(* **********************************)
...
fun is_value (Integer n) = true
  ...
  (* A memória é representada por uma lista de pares loc*int. As operações

    lookup : store * loc         -> int option
    update : store * (loc * int) -> store option

   retornam NONE se o endereço não está no domínio da memória.
  *)
...
fun update (s, (l,n)) = update' [] s (l,n)
  ...
  (* Definição da função que implementa um passo de avaliação

     reduce :  expr * store -> (expr * store) option 

  que recebe uma configuração (e,s) e retorna ou NONE, caso não
  haja transição, ou SOME (e',s'), se há transição (e,s) --> (e',s'). 
  *)
...
fun reduce (Integer n,s) = NONE
  ...
  (* Funçao de avaliação em vários passos

     evaluate :  expr * store -> (expr * store)  

  que recebe uma configuração (e,s) e retorna (e',s')
  tal que (e,s) -->* (e',s') -/->.  
...
(* pretty print da memória, ordenada por endereços *)
...
(* pretty print de configurações *)

fun prettyprintconfig (e,s) = 
 "< " ^ (prettyprintexpr e) ^ " , " ^ (prettyprintstore s) ^ " >"

(* realiza um sequencia de reduções imprimindo o estado inicial e o estado após
   cada passo
*)
